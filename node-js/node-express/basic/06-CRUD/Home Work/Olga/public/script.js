...
function addProduct(e) {
   e.preventDefault();
    ...
    let { name, picture, price } = e.target.children;
    name = name.value;
    ...
    fetch('/addproduct', {
        ...
        body: JSON.stringify({ name, picture, price })
    })
        .then(r => r.json())
        .then(data => {
            console.log(data)
...
closeUpdatePopup.addEventListener('click', function(){updatePopUp.style.display="none"})
}
...
function updateProduct(e, name){
    e.preventDefault();
    console.log(name)
    let { newName, newPicture, newPrice } = e.target.children;
    newName =  newName.value;
    newPicture = newPicture.value;
    newPrice = newPrice.value;
    let newProduct = {name: newName, picture: newPicture, price: newPrice}

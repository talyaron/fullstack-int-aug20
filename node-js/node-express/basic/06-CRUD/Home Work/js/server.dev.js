...
app.post('/add_user', IsUserAllredyExist, function (req, res, next) {
  ...
  if (pulledUserFromClaient.UserID.length > 0) {
    ...
    if (res.userExist) {
      res.send({
        OK: false,
        massege: "user named ".concat(pulledUserFromClaient.UserID, " is exist")
      });
    } else {
      TheUserDataBase.push(req.body);
      res.send({
        OK: true,
        massege: "user named ".concat(pulledUserFromClaient.UserID, " is added")
      });
    }
  } else {
    res.send({
...
function IsUserAllredyExist(req, res, next) {
  // mideleWare that check if user exist in the dataBase
  var result = TheUserDataBase.find(function (_ref2) {
...
app.post('/post', function (req, res) {
  ...
  products.push(req.body); //console.log(products);

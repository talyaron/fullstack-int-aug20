"use strict";

/******************************************
 *  Author : eyal shemuel
 *  Created On : Tue Dec 22 2020
 *  File : server.js.js
 *******************************************/
var express = require('express');

var bodyParser = require('body-parser');

var cookieParser = require('cookie-parser');

var app = express();

var formidable = require('formidable');

var port = process.env.PORT || 4000; // let TheUserDataBase = [{ id:"eyal", pass: "123" }];

var TheUserDataBase = [{
  UserID: 'eyal',
  role: 'admin',
  pass: '123'
}, {
  UserID: 'ben',
  role: 'public',
  pass: '1233'
}];
var products = [{
  ProductName: 'bar',
  image: 'data:image/jpeg;base64,',
  price: '4$'
}]; ///////app.use////////////////////////////////

app.use(cookieParser());
app.use(bodyParser.json());
app.use(express["static"]('public')); //////////////////////////////////////////////
//file upload////////////////////////////////////////////////////////////

app.get('/', function (req, res) {
  res.sendFile(__dirname + '/public/admin.html');
});
app.post('/', function (req, res) {
  var form = new formidable.IncomingForm();
  form.parse(req);
  form.on('fileBegin', function (name, file) {
    file.path = __dirname + '/public/image/' + file.name;
  });
  form.on('file', function (name, file) {
    console.log("Uploaded file", file.name);
  });
  res.sendFile(__dirname + '/public/admin.html');
}); /////////////////////////////////////////////////////////////////////////////
//login //////////////////////////////////////////////////////////////////////

app.post('/U_info', getUserfromedataBse, ISuserAuthorized, function (req, res, next) {
  var resultarry = TheUserDataBase.find(function (_ref) {
    var UserID = _ref.UserID;
    return UserID === req.body.UserID;
  });

  if (resultarry != undefined) {
    if (req.body.UserID == resultarry.UserID && req.body.pass == resultarry.pass) {
      console.log('passed');
      res.cookie('userAuthorized', res.userFromDatabase.role, {
        maxAge: 99999999999999999999
      });
      res.send({
        ok: true
      });
    } else {
      console.log('rejected');
      res.send({
        ok: false,
        ISuserAuthorized: "".concat(res.authorized)
      });
    }
  } else {
    res.send({
      ok: false
    });
    console.log('rong user name or password');
  }

  next();
}); /////////////////////////////////////////////////////////////////////
// add user//////////////////////////////////////////////////////////

app.post('/add_user', IsUserAllredyExist, function (req, res, next) {
  theUserEntersIsEmpty = true;
  IsUserAllredyExist = true;
  console.log(req.body);

  if (req.body.UserID.length > 0) {
    theUserEntersIsEmpty = false;

    if (res.userExist) {
      res.send({
        OK: false,
        massege: "user named ".concat(req.body.UserID, " is exist")
      });
    } else {
      TheUserDataBase.push(req.body);
      res.send({
        OK: true,
        massege: "user named ".concat(req.body.UserID, " is added")
      });
    }
  } else {
    res.send({
      OK: false,
      massege: 'you can not enter empty user'
    });
  }

  next();
}); ////////////////////////////////////////////////////////////////////
//mideleWares//////////////////////////////////////////////////////

function ISuserAuthorized(req, res, next) {
  res.authorized = 'forbidden';
  var userAuthorized = req.cookies.userAuthorized;

  if (userAuthorized === 'admin') {
    res.authorized = true;
  }

  next();
}

function getUserfromedataBse(req, res, next) {
  res.userFromDatabase = TheUserDataBase.find(function (_ref2) {
    var UserID = _ref2.UserID;
    return UserID === req.body.UserID;
  });
  next();
}

function IsUserAllredyExist(req, res, next) {
  // middleWare that check if user exist in the dataBase
  var result = TheUserDataBase.find(function (_ref3) {
    var UserID = _ref3.UserID;
    return UserID === req.body.UserID;
  }); // console.log(result)

  if (result != undefined) {
    console.log('user exist', result);
    res.userExist = true;
  } else {
    console.log('New user', result);
    res.userExist = false;
  }

  next();
} ///////////////////////////////////////////////////////////////////
//the client get (ask/read) from server thr products


app.get('/read', function (req, res) {
  res.send({
    ok: true,
    products: products
  });
}); ////////////////////////////////////////////////////////////////////////
//the client send/CREATE Product to the server /////////////////////

app.post('/post', function (req, res) {
  products.push(req.body);
  res.send({
    ok: true,
    products: products
  });
}); ////////////////////////////////////////////////////////////////////
//the client ask the server to delete somthing on the server (also called DELETE)////////////////////////

app["delete"]("/delete", function (req, res) {
  var ProductName = req.body.ProductName; //find the index of the user in the array

  var ProductIndex = products.findIndex(function (product) {
    return product.ProductName === ProductName;
  }); //remove the user from the array

  products.splice(ProductIndex, 1);
  console.log(products); //return the users

  res.send({
    ok: true,
    products: products
  });
}); //////////////////////////////////////////////////////////////////////////////////////////////////////////

app.listen(port, function () {
  console.log("Listen on port ".concat(port));
});